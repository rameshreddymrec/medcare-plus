...
import { Calendar, Video, MapPin, User, XCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl border border-light-border gap-4 shadow-sm">
        <div>
          <h2 className="text-2xl font-black text-slate-800">Practitioner Consultations</h2>
          <p className="text-sm text-slate-400">Track and manage your scheduled checkups and telehealth virtual rooms.</p>
        </div>
        <Link to="/doctors">
          <Button variant="primary">Book Doctor Appointment</Button>
        </Link>
      </div>
        ...
        <AnimatePresence mode="wait">
          {filteredApts.length === 0 ? (
            ...
            >
              ...
              <h4 className="text-sm font-semibold text-slate-500">No appointments logged</h4>
              <p className="text-xs text-slate-400 mt-1">Book consultations with top medical practitioners on our search portal.</p>
            </motion.div>
          ) : (
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="text-sm font-bold text-slate-800">{apt.doctorName}</h4>
                      <Badge variant="neutral" className="text-[9px] font-semibold">{apt.doctorSpecialty}</Badge>
                      <Badge 
                        variant={
                    ...
                    <div className="text-xs text-slate-400 flex flex-wrap gap-x-3 gap-y-1">
                      <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5 text-slate-400" /> {apt.date} • {apt.timeSlot}</span>
                      <span className="flex items-center gap-1"><User className="h-3.5 w-3.5 text-slate-400" /> Patient: {apt.patientName}</span>
                    </div>
                    <p className="text-[11px] text-slate-400 italic">Reason: "{apt.reason}"</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 w-full md:w-auto justify-end border-t md:border-t-0 pt-3 md:pt-0">
                  {apt.status === 'CONFIRMED' && apt.type === 'VIRTUAL' && (
                    <Link to={`/video-consultation/${apt.id}`}>
                      <Button size="sm" variant="accent" className="font-bold flex items-center gap-1.5">
                  ...
                  {apt.status === 'COMPLETED' && (
                    <Badge variant="success" className="px-3 py-1 font-bold text-[10px]">VISIT CONCLUDED</Badge>
                  )}
                  {apt.status === 'CANCELLED' && (
                    <Badge variant="danger" className="px-3 py-1 font-bold text-[10px]">CANCELLED</Badge>
                  )}
                </div>
              </motion.div>
            ))
